...
import { baseEvidence, isSameOrigin, makeResult, resolveUrl, safeGoto, takeScreenshot } from '../utils.js';

export async function checkHttps(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  ...
  return makeResult(
    ...
    isHttps ? 'PASS' : 'FAIL',
    ctx.websiteUrl,
    'The website must use HTTPS for sensitive workflows.',
    isHttps ? 'The site is served over HTTPS.' : `The site is served over an insecure scheme (${new URL(ctx.websiteUrl).protocol}).`,
    evidence,
    start
  );
}

export async function checkMixedContent(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  ...
  return makeResult(
    ...
    status,
    ctx.websiteUrl,
    'HTTPS pages must not depend on insecure HTTP resources.',
    status === 'FAIL' ? `Loaded ${insecureRequests.length} resource(s) over insecure HTTP.` : 'No insecure HTTP resources were loaded.',
    evidence,
    start
  );
}

async function attemptRoutes(browser: Browser, ctx: CheckContext, routes: string[]) {
  const context = await browser.newContext();
  const page = await context.newPage();
  const attempts: { route: string; url: string; status: number | null; finalUrl: string; redirected: boolean }[] = [];
...
export async function checkProtectedPage(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  ...
  return makeResult(
    ...
    status,
    routes.join(', '),
    'Protected pages must not be accessible using an unauthenticated browser session.',
    status === 'FAIL'
      ? `${stillAccessible.length} protected route(s) loaded without authentication: ${stillAccessible.map((a) => a.route).join(', ')}.`
      : 'All protected routes redirected/blocked an unauthenticated session.',
    evidence,
    start
  );
}

export async function checkUnauthorizedBehaviour(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  const routes = ctx.testContext.protected_routes ?? [];
  ...
  return makeResult(
    ...
    status,
    routes.join(', '),
    'Unauthorized users should be redirected, denied access, or otherwise blocked.',
    `${blocked.length}/${attempts.length} route(s) showed redirect/401/403 behaviour for an unauthenticated request.`,
    evidence,
    start
  );
}

export async function checkLogoutProtection(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  ...
  return makeResult(
    ...
    status,
    route ?? null,
    'After logout, the same browser session must not retain protected-page access.',
    !hasLogout ? 'No logout control could be found on the page.' : stillAccessible ? 'Protected page remained accessible after logout.' : 'Protected page was no longer accessible after logout.',
    evidence,
    start
  );
}

export async function checkUrlSensitiveInfo(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  const context = await browser.newContext();
  const page = await context.newPage();
  ...
  return makeResult(
    'SC-006',
    flagged.length > 0 ? 'FAIL' : 'PASS',
    ctx.websiteUrl,
    'URLs must not expose passwords, tokens, session IDs, or credentials.',
    flagged.length > 0 ? `${flagged.length} visited URL(s) contained a suspicious query parameter.` : 'No visited URL exposed an obvious credential-like query parameter.',
    evidence,
    start
  );
}

export async function checkPasswordFieldType(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  const context = await browser.newContext();
  const page = await context.newPage();
  ...
  return makeResult(
    ...
    status,
    target,
    'Password values must be entered through an appropriate password field.',
    status === 'FAIL' ? 'A password-like field was found that is not using type="password".' : `${passwordInputs} password field(s) correctly use type="password".`,
    evidence,
    start
  );
}

export async function checkVisibleSecrets(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  const context = await browser.newContext();
  const page = await context.newPage();
  await safeGoto(page, ctx.websiteUrl, ctx.timeoutMs);
  ...
  return makeResult(
    ...
    match ? 'FAIL' : 'PASS',
    ctx.websiteUrl,
    'Public pages must not visibly expose obvious passwords, API secrets, tokens, or credentials.',
    match ? 'Visible page text matched a pattern commonly associated with an exposed secret.' : 'No obviously exposed secret pattern was found in the visible page text (not a comprehensive secret scan).',
    evidence,
    start
  );
}

export async function checkAdminPageProtection(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  ...
  return makeResult(
    ...
    status,
    routes.join(', '),
    'Administrative pages must not be accessible to unauthorized or unauthenticated users.',
    status === 'FAIL' ? `${stillAccessible.length} admin route(s) were accessible without authentication: ${stillAccessible.map((a) => a.route).join(', ')}.` : 'All admin routes correctly blocked an unauthenticated session.',
    evidence,
    start
  );
}

export async function checkLoginWorkflow(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  ...
  try {
    ...
    if (stillHasPasswordField) ok = false;
  } catch (e: any) {
    await context.close();
  ...
  return makeResult(
    ...
    ok ? 'PASS' : 'FAIL',
    target,
    'Valid login should succeed; invalid login should be rejected.',
    steps.join('; '),
    evidence,
    start
  );
...
export async function checkRoleBasedAccess(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
...
export async function checkErrorInfoExposure(browser: Browser, ctx: CheckContext): Promise<RuleResult> {
  const start = Date.now();
  const context = await browser.newContext();
  const page = await context.newPage();
  ...
  return makeResult(
    ...
    exposed ? 'FAIL' : 'PASS',
    target,
    'Error pages must not expose stack traces, database details, or internal infrastructure information.',
    exposed ? 'The error page contained content resembling a stack trace or backend error detail.' : 'No obvious backend error detail was found on the error page.',
    evidence,
    start
  );
}
